...
# 파일 이름에 적힌 회계연도 — 「2024회계연도」 「'24회계연도」 「FY2025」 「2026년」
해찾기 = re.compile(r"(?:FY)?(20[0-2]\d)\s*(?:회계연도|년|년도)"
                  r"|['‘’ʼ](\d\d)\s*회계연도", re.I)   # 굽은 따옴표도 쓴다
...
def 요즘것(이름, 기준해):
    """옛 문서가 같은 페이지에 남아 있는 곳이 많다 — **이름에 적힌 해를 본다.**

    ⚠️ 2026-09-21 에 잡은 오류. 인천옹진군은 2015·2016·2017년 기금성과분석만 붙어 있는데
       「기금을 공시했다」로 세고 있었다. 해가 안 적힌 이름은 그대로 인정한다(대개 최신이다).
    """
    해 = 적힌해(이름)
    return 해 is None or 해 >= 기준해
...
def main():
    ...
    # 「요즘 것」의 잣대 — 결산공시는 한 해 늦게 나오므로 올해−2 까지 인정한다
    #   (2026년이면 2024회계연도 결산공시가 최신이다)
    기준해 = __import__('datetime').date.today().year - 2
    ...
    for r in dis['곳']:
        ...
        # ⚠️ 옛 문서로 「올렸다」가 되지 않게, **요즘 것만** 60조 항목 판정에 쓴다.
        요즘 = [f for f in 읽힌 if 요즘것(f, 기준해)]
        옛것 = len(읽힌) - len(요즘)
        걸린항목 = []
        if 요즘:
            글 = ' '.join(요즘)
    ...
    요약 = {
        ...
        '옛것있는곳': sum(1 for x in 곳 if x['옛것']),
        '기준해': 기준해,
