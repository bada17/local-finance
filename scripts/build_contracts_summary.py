...
#
# ⚠️⚠️ **업체를 이름으로만 묶는다 — 사업자번호가 자료에 없다.**
#    `WCEGCF` 출력 13칸을 다 받아도 번호가 없고, 지방재정365 146종 어디에도 없다(2026-09-21 확인).
#    나라장터(조달청) 계약정보에는 있지만 `.env` 의 `DATA_GO_KR_KEY` 가 비어 있어 아직 못 붙였다.
#    그래서 **법인격 표기만 다른 이름은 합치되, 한쪽에만 붙은 것은 합치지 않는다** —
#      합침   「태동건설(주)」 ＝ 「태동건설 주식회사」   (둘 다 법인이라고 적혀 있다)
#      안 합침 「중앙아이엔티 주식회사」 ≠ 「중앙아이엔티」 (개인사업자일 수 있다)
#    2025년 기준 합쳐지는 것 15,021개(1조원), 못 합치고 남는 헷갈림 4,646개(3,056억).
#
# ⚠️⚠️ **「계약 상대」가 업체가 아니라 조달청인 줄이 많다(2026-09-21 에 잡음).**
#    2025년에 **43곳은 1위가 조달청**이고 달성군은 43.1%다. 조달청을 거쳐 산 것이라
#    **진짜 업체는 이 자료에 없다**(나라장터에 있다). 지우지 않고 **따로 세어 화면이 가려 준다.**
#    ⚠️ 「조달」만 든 이름은 진짜 업체다 — 「영남레미콘사업(협)(조달)」처럼. **「조달청」으로 가른다.**
...
법인격 = re.compile(r'\(주\)|㈜|주식회사|\(유\)|유한회사|유한책임회사|합자회사|\(합\)|합명회사|'
                  r'농업회사법인|영농조합법인|사회적협동조합|\(사\)|사단법인|재단법인|\(재\)')
군더더기 = re.compile(r'[\s.,·\-_()]')


def 업체키(이름):
    """업체를 묶는 열쇠 — (법인격을 뗀 속이름, 법인이라고 적혀 있나).

    ⚠️ **둘째 칸을 빼면 안 된다.** 그것을 빼는 순간 개인사업자와 법인이 한 덩어리가 된다.
    """
    n = unicodedata.normalize('NFKC', 이름 or '')
    법인 = bool(법인격.search(n))
    return 군더더기.sub('', 법인격.sub('', n)), 법인
...
def main():
    ...
    셈 = collections.defaultdict(lambda: {
        ...
        '겹친건수': 0, '겹친금액': 0,
        '이름꼴': collections.defaultdict(collections.Counter),   # 열쇠 → {적힌 이름: 금액}
        '조달청건수': 0, '조달청금액': 0,
    ...
    for (cd, 해), s in 셈.items():
        ...
        def 보일이름(열쇠):
            """제일 많이 받은 표기를 대표로 쓴다. 다르게 적힌 것이 있으면 몇 가지인지도 알려준다."""
            꼴 = s['이름꼴'][열쇠]
            대표 = 꼴.most_common(1)[0][0]
            return 대표, len(꼴)

        # 「속이름은 같은데 한쪽만 법인격이 붙은」 헷갈리는 쌍 — 합치지 않고 세어만 둔다
        속별 = collections.Counter()
        for (속, 법인) in 업체:
            속별[속] += 1
        헷갈림 = sum(1 for 속, n in 속별.items() if n > 1)

        상위 = [(보일이름(k)[0], v[0]) for k, v in 차례[:5]]
        곳[cd][해] = {
            '건수': s['건수'],
            '금액': s['금액'],
            '수의건수': s['수의건수'],
            ...
            '조달청건수': s['조달청건수'],
            '조달청금액': s['조달청금액'],
            '조달청몫': round(s['조달청금액'] / s['금액'] * 100, 1) if s['금액'] else None,
            '다년건수': s['다년건수'],
            '다년금액': s['다년금액'],
        ...
        자세히[cd][해] = {
            ...
            '업체수': len(업체), '헷갈리는이름': 헷갈림,
            '조달청건수': s['조달청건수'], '조달청금액': s['조달청금액'],
            '종류': dict(s['종류'].most_common()),
            # [대표 이름, 다르게 적힌 이름 수, 금액, 건수, 수의금액, 조달청인가]
            '업체': [[*보일이름(k), v[0], v[1], v[2], 1 if '조달청' in 보일이름(k)[0] else 0]
                   for k, v in 차례[:상위N]],
        }
    ...
    조 = sum(v['조달청금액'] for c in 곳.values() for v in c.values())
    print(f'   계약 상대가 조달청인 것 {조/1e8:,.0f}억 ({조/max(1,총액)*100:.2f}%) '
          f'— 진짜 업체는 나라장터에 있다')
    헷 = sum(v['헷갈리는이름'] for c in 곳.values() for v in c.values())
    print(f'   속이름이 같은데 한쪽만 법인격이 붙어 못 합친 것 {헷:,}쌍 (사업자번호가 있으면 풀린다)')
